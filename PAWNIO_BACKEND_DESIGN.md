...
7. **Deleted claim (v1 §11.4 was wrong):** `SampleMatch` does **not** "reject torn multi-register snapshots" — it compares exactly one field, `FanCtrl` (`fanstuff.cpp:766-780`, the only test at `:770`). It detects a changed control byte between samples, nothing more. The defense against torn tach/temp data is the group hold above plus the EC's own transaction quiescence (`portio.cpp:78,155`); do not cite `SampleMatch` for it.
